...
## FortuneSpin (apps/fortune_wheel)

Журнал успешных вращений. Наследует `BaseDjangoModel`.

| Поле | Тип | Описание |
|------|-----|----------|
| `user` | FK → SystemUser (`PROTECT`) | Владелец вращения |
| `prize_apples` | PositiveInt | Фактически начисленный приз |
| `created_at` | inherited DateTime | Авторитетное время вращения и начало 240-часового интервала |

Индекс `(user, -created_at)` поддерживает поиск последнего результата.
Начисление `apple_balance` и создание строки выполняются в одной транзакции;
неуспешные и заблокированные попытки в таблицу не попадают. Django Admin
разрешает корректировать `user`, `prize_apples`, `is_active` и `created_at`
существующей строки; добавление и удаление строк, а также изменение `id` и
`updated_at` запрещены. Административная корректировка не пересчитывает
`SystemUser.apple_balance`.
