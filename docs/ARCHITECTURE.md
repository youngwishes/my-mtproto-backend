...
В админке `MTPRotoKey` доступна ручная синхронизация выбранных действующих
ключей на все активные VDS, включая unhealthy. Action ставит отдельную задачу
доставки для каждой пары ключ/сервер через существующий механизм с повторами;
неактивные, удалённые и истёкшие ключи пропускаются. Сообщение в админке
подтверждает постановку задач, а не завершение доставки.
