...
Confirm атомарно списывает сохранённое количество и продлевает тот же выбранный
ключ от `max(current_expiry, confirmation_at)`. Повтор подтверждённого ID
возвращает идентичный сохранённый `200` без нового списания или продления.
Изменённый/удалённый ключ, недостаточный после preview баланс либо чужой,
неизвестный или некорректный confirmation дают `400` без mutation; stale quote
