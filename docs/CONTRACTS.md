...
Оба endpoint защищены не `Bot-Auth-Token`, а заголовком
`Telegram-Init-Data` с исходной строкой `Telegram.WebApp.initData`. Backend
проверяет HMAC-подпись через `BOT_TOKEN`, наличие `user.id` и `auth_date` не
старше `FORTUNE_WHEEL_INIT_DATA_MAX_AGE_SECONDS` (default `3600`). Telegram ID
не принимается отдельным полем. Отсутствующие, просроченные и некорректно
подписанные данные возвращают `403`.

### POST /api/v1/fortune-wheel/status/

Тело запроса пустое. Возвращает состояние зарегистрированного пользователя без
изменения БД.
...
```json
{
  ...
  "can_spin": false,
  "last_prize": 25,
  "next_spin_at": "2026-09-08T12:00:00Z",
  "registration_url": null
}
```

До первого вращения `can_spin=true`, а оба поля результата равны `null`. Для
отсутствующего пользователя или непринятых условий `registered=false`,
`can_spin=false`, поля результата равны `null`, а `registration_url` содержит
URL бота.

### POST /api/v1/fortune-wheel/spin/

Тело запроса пустое. Backend повторно проверяет 240-часовой интервал,
атомарно начисляет выбранный приз и сохраняет вращение.

```json
{
  "prize_apples": 25,
  "spun_at": "2026-08-29T12:00:00Z",
  "next_spin_at": "2026-09-08T12:00:00Z"
}
```

Успех возвращает `200`. Ранний или параллельный повтор возвращает `409` с
последним сохранённым результатом:

```json
{
  "error": "Следующее вращение пока недоступно.",
  "last_prize": 25,
  "next_spin_at": "2026-09-08T12:00:00Z"
}
```

Непринятые условия дают `403` с `registration_url`. Временная DB-ошибка даёт
`503` и `{"error": "Не удалось выполнить вращение. Попробуйте ещё раз."}`.
Ни один неуспешный ответ не создаёт вращение и не начисляет яблоки.
