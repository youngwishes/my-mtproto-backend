...
Read-only проверяет, сохранено ли единое принятие пользовательского соглашения
и согласия на обработку персональных данных. Отсутствующий пользователь
возвращает `false`; endpoint не создаёт и не обновляет `SystemUser`.
...
`username` обязателен и принимается только как строка из ASCII-цифр. Пробелы в
начале или конце не нормализуются и возвращают `400 Bad Request`.
...
Сохранённое согласие возвращается тем же контрактом со значением `true`.
Нечисловой `username` возвращает `400 Bad Request`.

---

### POST /users/consent/accept/

Атомарно и идемпотентно фиксирует единое юридическое согласие. Для нового
Telegram ID создаёт ровно одного пользователя с
`legal_terms_accepted=true`. Для существующего пользователя меняет только этот
флаг и не перезаписывает сохранённые Telegram username и referrer.
...
| Поле | Тип | Описание |
|------|-----|----------|
| `username` | numeric string | Обязательный Telegram ID пользователя |
| `telegram_username` | string? | Необязательный Telegram username |
| `invited_from_username` | numeric string? | Необязательный Telegram ID пригласившего |
...
Нечисловой `username` или `invited_from_username`, а также self-referrer
возвращают `400 Bad Request` без частично созданного пользователя. Повторный и
конкурентный accept оставляют одну запись и не меняют первоначальный referrer.
Пробелы в начале или конце numeric-string полей также возвращают `400 Bad
Request` и не приводят к записи.
...
Read-only проверяет доступность бесплатного периода и определяет его
длительность для уже существующего пользователя с принятым юридическим
согласием. Endpoint больше не регистрирует пользователя, не обновляет
`telegram_username` и не сохраняет referrer.
...
**Ошибки:** `LegalTermsNotAccepted` (`400 Bad Request`) — пользователь
отсутствует либо его `legal_terms_accepted=false`. Ошибка не создаёт и не
