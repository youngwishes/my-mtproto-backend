...
1. обновляет in-memory профиль;
2. добавляет или обновляет пользователя управляемого Xray inbound;
3. возвращает `200` только после успешного применения Xray-операции.
