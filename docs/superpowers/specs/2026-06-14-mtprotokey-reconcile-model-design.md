...
- **Шаг 7 (миграции + дроп колонок).** Миграция `0019_remove_mtprotokey_node_number_and_more`: `RemoveField` `MTPRotoKey.vds`/`node_number`/`tls_domain` + `VDSInstance.user_limit`. Из модели убраны: `VDSQuerySet` (методы `order_by_population`/`get_least_populated`) — менеджер теперь `ActiveQuerySet.as_manager()`; `VDSInstance.is_available`; FK `vds` (вместе с reverse-relation `VDSInstance.keys`). Селектор `get_least_populated_vds` удалён. `admin.py`: из `VDSInstanceAdmin` убраны колонки `user_limit`/`active_keys_count`/`not_active_keys_count` и методы (использовали `obj.keys`); из `MTPRotoKeyAdmin` убраны `vds` из `list_display`/`list_filter`/`list_select_related`. `factories.py`: убраны `vds`/`tls_domain`/`user_limit`. Тесты: удалены `TestVDSQuerySet`/`TestGetLeastPopulatedVds`; `vds=`/`tls_domain=`/`vds_id`-ассерты вычищены из ~22 тест-файлов (массовая правка скриптом + ручная доводка). Прод-значение `TLS_DOMAIN == "beatvault.ru"` подтверждено ранее — дроп `tls_domain` безопасен (`get_secret_token()` берёт домен из settings с Шага 2).
