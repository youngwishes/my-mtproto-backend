...
Scope Contract и создаваемые ролями `business.md`, `architecture.md`, `plan.md`,
`acceptance.md` хранятся в `.codex/work/<feature-slug>/`. Это игнорируемые Git
рабочие материалы: они координируют роли во время разработки, но не входят в
commit, Pull Request или постоянную документацию. Не копировать в них большие
фрагменты production-кода, полные логи, хэши дерева и историю закрытых revision.
...
2. Создать рабочий каталог `.codex/work/<feature-slug>/` и хранить Scope Contract
   и промежуточные артефакты только в нём.
3. Проверить `git status --short`, текущую ветку и последние изменения. Не
   перезаписывать и не включать в commit чужие незавершённые изменения.
4. Обновить локальный `main` без переписывания истории и создать от него ветку
...
5. Проверить доступность GitHub CLI командой `gh auth status`. Отсутствие `gh`
   или авторизации является блокером публикации PR и не разрешает fallback на
   прямой push в `main`.
6. Найти существующие сервисы, selectors, DTO, exceptions, factories и тестовые
   паттерны, которые можно переиспользовать.
7. Сверить постановку, критерии приёмки, затронутые контракты и риски с текущим
   Scope Contract. Не добавлять найденные риски в требования без
   `scope_change_request`.
8. Для рискованных изменений определить обратную совместимость миграций и
...
6. Убедиться, что временные материалы из `.codex/work/`, `docs/features/`,
   `docs/superpowers/` и `superpowers/` не отслеживаются Git и отсутствуют в diff.

## 4. Публикация Pull Request

После зелёных проверок агент самостоятельно:

1. Формирует по утверждённому Scope Contract компактный Review Brief в описании
   PR: цель, наблюдаемое поведение, non-goals, критерии приёмки, проверки, риски
   и deploy impact. Не копирует туда рабочий план, task packets или полные логи.
2. Добавляет только относящиеся к фиче файлы и проверяет отсутствие временных
   агентских артефактов в index и diff.
3. Создаёт осмысленный commit в feature-ветке; commit в `main` запрещён.
4. Выполняет push feature-ветки и открывает Pull Request с base `main`.
...
2. Reviewer в read-only режиме читает Review Brief из описания PR, актуальные
   канонические документы, полный diff и checks через `gh pr view`, `gh pr diff`
   и `gh pr checks`. Временные материалы из `.codex/work/` он не читает. Затем
   reviewer публикует один структурированный комментарий через
...
3. Каждая находка классифицируется как `blocking_in_scope`,
   ...
   ссылка на критерий или non-goal из Review Brief либо конкретную регрессию из
   diff.
