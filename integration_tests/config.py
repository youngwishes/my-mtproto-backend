"""Конфигурация интеграционного харнесса (всё через env).

Чёрнобоксовые e2e-тесты гоняются против локального backend-стека с тестовой БД
и выделенного удалённого VDS API. В прод — НЕЛЬЗЯ.
"""
...
VDS_VERIFY_URLS: list[str] = [
    ...
    for u in os.environ.get(
        "INTEG_VDS_VERIFY_URLS", "http://31.77.148.123:8080"
    ).split(",")
    if u.strip()
]

# Значение, которое пишем в VDSInstance.internal_ip_address / .port, чтобы
# celery-контейнер дотянулся до выделенного тестового VDS API.
VDS_INTERNAL_IP: str = os.environ.get("INTEG_VDS_INTERNAL_IP", "31.77.148.123")
...
WAIT_TIMEOUT: float = float(os.environ.get("INTEG_WAIT_TIMEOUT", "75"))
