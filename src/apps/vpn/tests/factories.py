...
class VPNAccessNodeRevisionEvidenceFactory(factory.django.DjangoModelFactory):
    access = factory.SubFactory(VPNAccessFactory)
    node = factory.SubFactory(VPNNodeFactory)
    revision = factory.LazyAttribute(lambda obj: obj.access.desired_revision)
    status = VPNApplyStatus.PENDING

    class Meta:
        model = VPNAccessNodeRevisionEvidence
