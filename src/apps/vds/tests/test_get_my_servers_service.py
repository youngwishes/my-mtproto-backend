...
class TestGetMyServersService(TestCase):
    ...
    def test_returns_all_active_vds_links_for_user_with_key(self, mock_log) -> None:
        ...
        links_by_location = {
            server.location: server.proxy_link for server in result.servers
        }
        self.assertEqual(
            links_by_location,
            {
                "🇳🇱 Нидерланды": (
                    "tg://proxy?server=nl1.mtprotokeys.com&port=443&secret="
                    "eetesttoken746c732d612e6578616d706c65"
                ),
                "🇩🇪 Германия": (
                    "tg://proxy?server=de1.mtprotokeys.com&port=443&secret="
                    "eetesttoken746c732d622e6578616d706c65"
                ),
            },
        )
        secrets = [link.rsplit("&secret=", maxsplit=1)[1] for link in links_by_location.values()]
