...
class AddKeyToAnotherVdsInfraService:
    def __call__(self, *, exclude: int, username: str, secret: str) -> None:
        servers = get_other_active_vds_instances(exclude_pk=exclude)
        for server in servers:
            self._add_key_to_server(server=server, username=username, secret=secret)

    def _add_key_to_server(self, *, server: VDSInstance, username: str, secret: str) -> None:
        try:
...
def get_add_key_to_another_vds_instances_service() -> AddKeyToAnotherVdsInfraService:
    return AddKeyToAnotherVdsInfraService()
